...
gameWindow.moveGameWindowPositionTo = function (x, y) {
	if(gameContent.grid.length > 0) {
		this.x = Math.max(0, Math.min(Math.max(0, gameContent.grid[0].length - this.width), x));
		this.y = Math.max(0, Math.min(Math.max(0, gameContent.grid.length - this.height), y));
	}
}
