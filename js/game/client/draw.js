		...
		if(element.position.x >= gameWindow.x 
		  ...
		  && element.position.y <= gameWindow.y + gameWindow.height) {
			if(element.isSelected) {
				...
				this.ctx.strokeRect(-1 + (element.position.x - gameWindow.x) * gameWindow.PIXEL_BY_NODE - gameWindow.PIXEL_BY_NODE * parseInt(element.shape[0].length / 2),
							  -1 + (element.position.y - gameWindow.y) * gameWindow.PIXEL_BY_NODE - gameWindow.PIXEL_BY_NODE * parseInt(element.shape.length / 2),
							  element.shape[0].length * gameWindow.PIXEL_BY_NODE + 2,
							  element.shape.length * gameWindow.PIXEL_BY_NODE + 2);
			}
			this.ctx.fillStyle = element.color;
			this.ctx.fillRect((element.position.x - gameWindow.x) * gameWindow.PIXEL_BY_NODE - gameWindow.PIXEL_BY_NODE * parseInt(element.shape[0].length / 2),
							  (element.position.y - gameWindow.y) * gameWindow.PIXEL_BY_NODE - gameWindow.PIXEL_BY_NODE * parseInt(element.shape.length / 2),
							  element.shape[0].length * gameWindow.PIXEL_BY_NODE,
							  element.shape.length * gameWindow.PIXEL_BY_NODE);
			...
			if(element.family == gameData.FAMILIES.building) {
				if(element.queueProgression > 0) {
					...
					this.ctx.fillRect((element.position.x - gameWindow.x) * gameWindow.PIXEL_BY_NODE - gameWindow.PIXEL_BY_NODE * parseInt(element.shape[0].length / 2),
							   	 (element.position.y - gameWindow.y) * gameWindow.PIXEL_BY_NODE  - gameWindow.PIXEL_BY_NODE * parseInt(element.shape.length / 2),
							   	  element.queueProgression / 100 * gameWindow.PIXEL_BY_NODE * element.shape[0].length, 5);
				}
				if(element.constructionProgress < 100) {
					this.ctx.fillStyle = this.grd;
					this.ctx.fillRect((element.position.x - gameWindow.x) * gameWindow.PIXEL_BY_NODE - gameWindow.PIXEL_BY_NODE * parseInt(element.shape[0].length / 2),
							   	 (element.position.y - gameWindow.y) * gameWindow.PIXEL_BY_NODE  - gameWindow.PIXEL_BY_NODE * parseInt(element.shape.length / 2),
							   	  element.constructionProgress / 100 * gameWindow.PIXEL_BY_NODE * element.shape[0].length, 5);
				}
				for(var p in element.queue) {
					var stuff = element.queue[p];
					this.ctx.fillStyle = '#ff0';
					this.ctx.fillRect((element.position.x - gameWindow.x) * gameWindow.PIXEL_BY_NODE + p * gameWindow.PIXEL_BY_NODE - gameWindow.PIXEL_BY_NODE * parseInt(element.shape[0].length / 2), 
