...
*/
AI.searchForNewEnemy = function (unit) {
	var nearestEnemy = mapLogic.getNearestEnemy(unit);
...
*/
AI.targetReaction = function (target, attacker) {
	if (gameData.ELEMENTS[target.f][target.r][target.t].isBuilder) {
		//flee if it is a builder
		var around = tools.getTilesAroundElements(target);
		if (around.length > 0) {
